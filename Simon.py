...
class Simon() :
    def __init__(self, master):
        ...
        self.game_canvas = tkinter.Canvas(
                                          self.master,
    ...
    def check_choice(self):
        rect_id = self.game_canvas.find_withtag("current")[0]
        rect_index = self.rectangle_ids.index(rect_id)
        color = self.idle_colors[rect_index]
    ...
    def draw_canvas(self):
        ...
        for index, color in enumerate(self.current_colors): # Iterate over the colors in the list drawing each of the rectangles their respective place.
            if index <= 1:
                rectangle = self.game_canvas.create_rectangle(
                ...
                rectangle = self.game_canvas.create_rectangle(
                                        ...
                                        self.master.winfo_height() / 2,
                                        fill = color, outline = color)
            self.rectangle_ids.append(rectangle)
